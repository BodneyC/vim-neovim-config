...
function M.rg(q, b)
  ...
  vim.fn['fzf#vim#grep'](
    rg_cmd .. vim.fn.shellescape(q),
    1,
    vim.fn['fzf#vim#with_preview'](fzf_opts),
    b
  )
end
...
function M.rg_under_cursor()
  ...
  if string.match(w, '^[_a-zA-Z0-9]+$') then
    local fzf_opts = { down = '20%', options = { '--margin', '0,0' } }
    vim.fn['fzf#vim#grep'](rg_cmd .. vim.fn.shellescape(w), 1, vim.fn['fzf#vim#with_preview'](fzf_opts), 0)
  else
